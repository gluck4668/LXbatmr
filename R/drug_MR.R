...
#-----pval, beta, se, eaf 要转换成数字 numeric 格式-----
pval_sit <- grep(pval_exp,names(exp_df))
exp_df[,pval_sit] <- as.numeric(exp_df[,pval_sit])

beta_sit <- grep(beta_exp,names(exp_df))
exp_df[,beta_sit] <- as.numeric(exp_df[,beta_sit])

se_sit <- grep(se_exp,names(exp_df))
exp_df[,se_sit] <- as.numeric(exp_df[,se_sit])

eaf_sit <- grep(eaf_exp,names(exp_df))
exp_df[,eaf_sit] <- as.numeric(exp_df[,eaf_sit])
...
#-----pval, beta, se, eaf 要转换成数字 numeric 格式-----
pval_out_sit <- grep(pval_out,names(out_df))
out_df[,pval_out_sit] <- as.numeric(out_df[,pval_out_sit])

beta_out_sit <- grep(beta_out,names(out_df))
out_df[,beta_out_sit] <- as.numeric(out_df[,beta_out_sit])

se_out_sit <- grep(se_out,names(out_df))
out_df[,se_out_sit] <- as.numeric(out_df[,se_out_sit])

eaf_out_sit <- grep(eaf_out,names(out_df))
out_df[,eaf_out_sit] <- as.numeric(out_df[,eaf_out_sit])
...
#---save the results-----------------------------------------------------------
drugMR_mr_all <- rbind(drugMR_mr_all,mr_OR)

ivw_set <- subset(mr_OR,method=="Inverse variance weighted")
drugMR_ivw_all <- rbind(drugMR_ivw_all,ivw_set)
...
message(paste0("Number ",z, " of ",length(out_files)," outcome data was done."))

} # foreach(z=1:length(out_files),.errorhandling = "pass") end

message(paste0("Number ",y, " of ",nrow(target_gene_df)," target genes was done."))

} #foreach(y=1:nrow(target_gene_df),.errorhandling = "pass") end

print(paste0("Number ",x, " of ",length(exp_files)," exposure data was done."))
