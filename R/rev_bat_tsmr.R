...
#-------creating dir-----------------------
dir_file=paste0(exp_data_file," results (Rev_mr)")
...
Rev_mr_result_all = data.frame() # including all data
Rev_mr_result_significant = data.frame() # including data with p<0.05
...
if(file_type=="txt"){
  out_df <-read.table(out_data_file,header = T,sep = "\t")} else {
     if(file_type=="gz")
...
Rev_mr_result_all <- rbind(Rev_mr_result_all,res_or)

if(res_or$pval<0.05){
  Rev_mr_result_significant <- rbind(Rev_mr_result_significant,res_or) }
...
write.xlsx(Rev_mr_result_all,paste0(dir_file,"/rev_mr_result_all .xlsx"))

if(nrow(Rev_mr_result_significant)>0){
   write.xlsx(Rev_mr_result_significant,paste0(dir_file,"/rev_mr_result_significant.xlsx")) }
