...
#plot <- add_border(plot, part = "header", row = c(1,4,7,10,13,16,19), gp = gpar(lwd =1))
#设置字体大小和让文字居中
...
plot_file <- paste0(dir_one, "/",or_id,"_forest_plot.png")
...
#-----------------------------------
  or_df <- or_data_merg[(or_data_merg$method %in% mr_method),]
  or_df$method[(or_df$method %in% "Inverse variance weighted")] <- "IVW"
  or_df$pval <- as.numeric(or_df$pval)
...
#对数据进行整理
  ...
  plot_file <- paste0(dir_all, "/",or_id,"_forest_merge_plot.png")
