...
if(is.null(samplesize_info))
   ...
   meta_info <- meta_gwas_id} else {
     ...
     if(all(is.na(x2)))
      ...
      if(!not_idx=="")
      warning (paste0("'",not_idx,"'"," was not in the samplesize file '",samplesize_info,"', Please check it. "))
    } else
    warning(paste0(" The smaplesize file '",samplesize_info, "' was not existed. "))

 } #---esle end---

not_smaple_info <- any( !file.exists(samplesize_info), !any(grepl("samplesi",names(meta_info))), !not_idx=="" )
if(not_smaple_info){
   warning (paste0("The samplesize was not provided, and it will get from GWAS Catalog online. "))
  data_list = NA
  data_dir= data_dir
  bat_get_gwasinfo(data_dir,data_list)
  meta_info <-read.xlsx(paste0("gwas catalog information/",data_dir,"_gwas_id_info.xlsx"))
  }

  meta_info <- meta_info %>% data.frame()
  sam_sit <- grep("sample",names(meta_info),ignore.case = T)[1]
