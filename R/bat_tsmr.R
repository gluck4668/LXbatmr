...
{exp_list <- dir(exp_data_dir) %>% as.data.frame()
  colnames(exp_list)[1] <- "id"
    ...
    exp_list <- read.xlsx(exp_data_dir)
...
file_type <- tools::file_ext(exp_list[x,1]) %>% tolower() # tools::file_ext查看文件扩展名
...
write.xlsx(mr_all,paste0(dir_file,"/",out_data_name,"_outcome_mr_all.xlsx"))
write.xlsx(heterogeneity_test,paste0(dir_file,"/",out_data_name,"_heterogeneity_test.xlsx"))
write.xlsx(pleiotropy_test,paste0(dir_file,"/",out_data_name,"_pleiotropy_test.xlsx"))
...
if(nrow(mr_all_significant)>0){
  ...
  write.xlsx(mr_all_significant,paste0(dir_file,"/",out_data_name,"_outcome_mr_all_significant.xlsx"))
  write.xlsx(mr_ivw_significant,paste0(dir_file,"/",out_data_name,"_outcome_mr_ivw_significant.xlsx"))
