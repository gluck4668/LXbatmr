...
#-------------------------------------
  ...
  #多水平校验，这里是没有多水平效应的
  pleio<- mr_pleiotropy_test(harm_df)
  pleiotropy_test <- rbind(pleiotropy_test,pleio)

#-------------------------------------
 is.het = all(!is.na(heterogeneity_test$Q_pval),heterogeneity_test$Q_pval<0.05)
 is.ple = all(!is.na(pleiotropy_test$pval),pleiotropy_test$pval<0.05)
...
#-----------save files---------------
  ...
  write.xlsx(pleio, paste0(mr_save_file,"_pleiotropy_test.xlsx"))
...
if(nrow(mr_all_significant)>0){
  ...
  is.plei <- all(!is.na(pleiotropy_test$pval<0.05),any(pleiotropy_test$pval<0.05))
  if(is.plei)
