...
print(paste0("Number ",x, " of ",nrow(exp_list)," exposure data is being processed..."))
...
foreach(y=c(1:length(outcomeID)), .errorhandling = "pass") %do% {

   message(paste0("Number ",y, " of ",length(outcomeID)," outcome data is being processed..."))

   gwas_df <- fread(outcomeID[y])
   ...
   #-----------添加结局样本数量----------------------------------------#
   id02 <- str_extract(outcomeID[y],"(?<=\\/)[^/]+$") %>% str_extract(".*?(?=\\.)")
   ...
   message(paste0("Number ",y, " of ",length(outcomeID),"  outcome data was completed"))

   } # foreach outcome gwas end

#------------------------------------------

  print(paste0("Number ",x, " of ",nrow(exp_list)," exposure data was done."))
