...
if(!is.na(samplesize_info)){
  ...
  if(file.exists(samplesize_info)){
       sam_exp <-read.xlsx(samplesize_info)

       if(!any(grepl("samples",names(sam_exp))))
       warning(paste0(" '",samplesize_info,"' did not include the samplesize information. Please check it."))

       not_idx <- paste0(expid_list[!expid_list %in% sam_exp[,1]],collapse = ", ")
       if(!not_idx=="")
       warning (paste0("'",not_idx,"'"," was not in the samplesize file '",samplesize_info,"', Please check it. "))

       not_smaple_info <- any( !file.exists(samplesize_info), !any(grepl("samples",names(sam_exp))), !not_idx=="" )
